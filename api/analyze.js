...
export default async function handler(req, res) {
  ...
  if (!isCron) {
    ...
    if (user.role === 'viewer') return res.status(403).json({ error: 'View-only access. Analysis runs automatically each morning.' });
  }

  const allPortfolios = JSON.parse(process.env.PORTFOLIOS || '[]');

  if (isCron) {
    // Cron: run analysis for all portfolios
    const results = [];
    for (const pf of allPortfolios) {
      try {
        const result = await runAnalysis(pf);
        results.push({ id: pf.id, name: pf.name, verdict: result.verdict });
      } catch (e) {
        results.push({ id: pf.id, error: e.message });
      }
    }
    return res.status(200).json({ cron: true, results });
  }

  // Frontend: run specific portfolio
  const portfolioId = req.body?.portfolioId;
  const pf = allPortfolios.find(p => p.id === portfolioId) || allPortfolios[0];
...
- Reference actual price data. Think in weeks/months.`;
  ...
  // 6. Send email to all users who have access to this portfolio and have notify: true
  ...
  if (EMAILJS_SERVICE && EMAILJS_TEMPLATE && EMAILJS_PUBLIC_KEY) {
    const subject = `Vitru [${pf.name}] — ${needsAction ? 'ACTION REQUIRED' : 'Holding Steady'}`;
    const message = `Vitru Portfolio: ${pf.name} (${strategy}) — ${today}
...
Generated by Vitru LLC`;
    ...
    // Find all users who should receive notifications for this portfolio
    const notifyUsers = users.filter(u =>
      u.notify !== false &&
      (!u.portfolios || u.portfolios.length === 0 || u.portfolios.includes(pfId))
    );
      ...
      try {
        await fetch('https://api.emailjs.com/api/v1.0/email/send', {
          ...
          body: JSON.stringify({
            ...
            template_params: { to_email: user.email, subject, message },
          }),
        });
        emailSent = true;
      } catch (e) {
